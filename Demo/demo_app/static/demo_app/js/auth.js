/* =====================================================================
   DJANGO-BASED AUTHENTICATION FOR SMARTLEARN - FIXED VERSION
   ===================================================================== */
...
// ===== HELPER: SHOW ERROR MESSAGE (IMPROVED) =====
...
// ===== HELPER: SHOW SUCCESS MESSAGE (IMPROVED) =====
    ...
    if (data.success) {
      ...
      setTimeout(() => {
        ...
        if (typeof window.loadSearchHistory === 'function') {
          ...
          console.warn('⚠️ loadSearchHistory function not found');
        }
...
async function handleSignUp(name, email, password) {
  ...
  try {
    ...
    if (data.success) {
      // Save user to sessionStorage WITH streak
      if (data.user) {
        const userData = {
          ...data.user,
          streak: data.streak || null
      ...
      setTimeout(() => {
        ...
        // Show welcome streak notification
        if (data.streak && data.streak.current_streak > 0) {
          setTimeout(() => {
            if (typeof showNotification === 'function') {
